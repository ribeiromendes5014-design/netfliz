...
class Series(models.Model):
    ...
    DISPLAY_CATEGORY_SERIES = "series"
    DISPLAY_CATEGORY_MININOVELA = "mininovela"
    DISPLAY_CATEGORY_CHOICES = (
        (DISPLAY_CATEGORY_SERIES, "Séries"),
        (DISPLAY_CATEGORY_MININOVELA, "Mininovelas"),
    )
    display_category = models.CharField(
        max_length=16,
        choices=DISPLAY_CATEGORY_CHOICES,
        default=DISPLAY_CATEGORY_SERIES,
        help_text="Escolha onde esta série deve aparecer no catálogo.",
    )
...
class Video(models.Model):
    ...
    CATEGORY_TV = "tv"
    CATEGORY_MININOVELA = "mininovela"
    CATEGORY_CHOICES = (
    ...
    genre = models.CharField(
        ...
        blank=True,
        help_text="Gênero principal (filmes e séries).",
    )
    ...
    def save(self, *args, **kwargs):
        if not self.slug:
            self.slug = _generate_unique_slug(Video, self.title, instance=self)

        if self.series and self.category == self.CATEGORY_MININOVELA:
            if self.series.display_category != Series.DISPLAY_CATEGORY_MININOVELA:
                self.series.display_category = Series.DISPLAY_CATEGORY_MININOVELA
                self.series.save()
