...
class VideoForm(forms.ModelForm):
    ...
    def clean(self):
        ...
        season_number = cleaned.get("season_number")
        episode_number = cleaned.get("episode_number")

        needs_series = (
            category in [Video.CATEGORY_SERIES, Video.CATEGORY_MININOVELA]
            or season_number
            or episode_number
        )
        if needs_series and not series:
            self.add_error(
                "series",
                ValidationError(
                    "Associe este vídeo a uma série cadastrada para que temporadas e episódios sejam agrupados."
                ),
            )
...
class SeriesForm(forms.ModelForm):
    display_category = forms.ChoiceField(
        choices=Series.DISPLAY_CATEGORY_CHOICES,
        label="Seção",
        initial=Series.DISPLAY_CATEGORY_SERIES,
        help_text="Defina se a série aparece na seção Séries ou na seção Mininovelas do catálogo.",
    )
    ...
    class Meta:
        ...
        labels = {
            ...
            "cover_url": "URL da capa",
            "display_category": "Seção",
            "is_active": "Disponível",
        }
        help_texts = {
            "cover_url": "Coloque uma capa representativa para a série.",
            "display_category": "Escolha onde a série deve aparecer no catálogo.",
