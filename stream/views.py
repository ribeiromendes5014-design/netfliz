...
class TenantPortalView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        self._continue_movies = [video for video in continue_videos if not video.series_id]
        self._continue_series = [video for video in continue_videos if video.series_id]
        series_payload = self._build_series_data(tenant, all_videos, progress_map)
        self._series_list = series_payload
        self._series_payload_json = json.dumps(series_payload, cls=DjangoJSONEncoder)
        ...
        movie_videos = [video for video in standalone_videos if video.category != Video.CATEGORY_TV]
        self._tv_channels = tv_channels
        self._movies = movie_videos
        return movie_videos
    ...
    def _build_series_data(self, tenant, all_videos, progress_map):
        ...
        series_payload = []
        series_qs = Series.objects.filter(tenant=tenant, is_active=True).order_by("title")
