...
class TenantPortalView(LoginRequiredMixin, ListView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context["mininovela_series"] = getattr(self, "_mininovela_series", [])
        context["series_payload"] = getattr(self, "_series_payload_json", "{}")
        context["continue_mininovela"] = getattr(self, "_continue_mininovela", [])
        ...
        context["mininovela_videos"] = getattr(self, "_mininovela_videos", [])
        return context
    ...
    def _apply_portal_payload(self, payload):
        ...
        self._continue_mininovela = payload.get("continue_mininovela", [])
        series_models_by_category = payload.get("series_models_by_category", {})
        self._series_list = series_models_by_category.get(
            Series.DISPLAY_CATEGORY_SERIES,
            [],
        )
        self._mininovela_series = series_models_by_category.get(
            Series.DISPLAY_CATEGORY_MININOVELA,
            [],
        )
        ...
        self._mininovela_videos = payload.get("mininovela_videos", [])
    ...
    def _build_portal_payload(self, tenant):
        ...
        continue_series = [video for video in continue_videos if video.series_id and video.category != Video.CATEGORY_MININOVELA]
        continue_tv = [video for video in continue_videos if video.category == Video.CATEGORY_TV]

        series_payload_by_category, series_models_by_category = self._build_series_data(
            tenant,
            all_videos,
            progress_map,
        )
        standalone_videos = [video for video in all_videos if not video.series_id and video.category != Video.CATEGORY_MININOVELA]

        tv_channels = [video for video in standalone_videos if video.category == Video.CATEGORY_TV]
        mininovela_videos = [
            video
            for video in all_videos
            if video.category == Video.CATEGORY_MININOVELA and not video.series_id
        ]
        videos_by_genre = {}
        ...
        continue_mininovela = [
            video for video in continue_videos if video.category == Video.CATEGORY_MININOVELA
        ]
        ...
        return {
            ...
            "continue_mininovela": continue_mininovela,
            "series_models_by_category": series_models_by_category,
            "series_payload_json": json.dumps(series_payload_by_category, cls=DjangoJSONEncoder),
            "tv_channels": tv_channels,
            "mininovela_videos": mininovela_videos,
    ...
    def _build_series_data(self, tenant, all_videos, progress_map):
        ...
        series_payload_by_category = {key: [] for key in category_keys}
        series_models_by_category = {key: [] for key in category_keys}
        if not tenant:
            return series_payload_by_category, series_models_by_category
        video_by_series = {}
        for video in all_videos:
        ...
        for series in series_models:
            ...
            if series.display_category == Series.DISPLAY_CATEGORY_MININOVELA:
                series_payload_by_category[Series.DISPLAY_CATEGORY_MININOVELA].append(payload_entry)
                series_models_by_category[Series.DISPLAY_CATEGORY_MININOVELA].append(series)
            else:
                series_payload_by_category[Series.DISPLAY_CATEGORY_SERIES].append(payload_entry)
                series_models_by_category[Series.DISPLAY_CATEGORY_SERIES].append(series)
        return series_payload_by_category, series_models_by_category
...
class WatchVideoView(TemplateView):
    ...
    def get(self, request, slug, video_slug):
        ...
        next_video_url = None
        if video.series:
            series_title = video.series.title or ""
            episodes_qs = Video.objects.filter(
                series=video.series,
                is_public=True,
            ).filter(
                models.Q(blocked_tenants__isnull=True) | ~models.Q(blocked_tenants=tenant)
            ...
            episodes_sorted = sorted(
                episodes,
                key=lambda entry: (
                    entry.season_number if entry.season_number is not None else 1,
                    entry.episode_number if entry.episode_number is not None else 0,
                    entry.created_at or timezone.now(),
                ),
            )

            def build_episode_url(target_video, autoplay=False):
                url = reverse(
                    "stream:tenant-watch",
                    kwargs={"slug": slug, "video_slug": target_video.slug},
                )
                if autoplay:
            ...
            current_index = None
            for idx, entry in enumerate(episodes_sorted):
                season_number = entry.season_number if entry.season_number is not None else 1
                episode_number = entry.episode_number if entry.episode_number is not None else idx + 1
                if entry.pk == video.pk:
                    current_index = idx
                    episode_label = f"S{season_number:02d} E{episode_number:02d}"
                    break
            if current_index is not None:
                if current_index > 0:
                    prev_video = episodes_sorted[current_index - 1]
                    prev_video_url = build_episode_url(prev_video, autoplay=True)
                if current_index + 1 < len(episodes_sorted):
                    next_video = episodes_sorted[current_index + 1]
                    next_video_url = build_episode_url(next_video, autoplay=True)
...
def _clear_portal_cache():
    ...
    cache_keys = [f"tenant_portal:{tenant.slug}" for tenant in tenants]
...
class OwnerSeriesPermissionMixin(UserPassesTestMixin):
    def test_func(self):
        series = self.get_object()
        owner_tenant = get_or_create_owner_tenant(self.request.user)
        return self.request.user.is_active and self.request.user.is_staff and series.tenant == owner_tenant


class OwnerSeriesEditView(LoginRequiredMixin, OwnerSeriesPermissionMixin, UpdateView):
    model = Series
    form_class = SeriesForm
    template_name = "stream/series_form.html"
...
class OwnerSeriesDeleteView(LoginRequiredMixin, OwnerSeriesPermissionMixin, DeleteView):
    model = Series
    template_name = "stream/series_confirm_delete.html"
