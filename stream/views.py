...
class TenantPortalView(LoginRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        self._continue_movies = [video for video in continue_videos if not video.series_id and video.category == Video.CATEGORY_MOVIE]
        self._continue_series = [video for video in continue_videos if video.series_id]
        self._continue_tv = [video for video in continue_videos if video.category == Video.CATEGORY_TV]

        series_payload, series_models = self._build_series_data(tenant, all_videos, progress_map)
        self._series_list = series_models
        ...
        self._videos_by_genre = videos_by_genre
